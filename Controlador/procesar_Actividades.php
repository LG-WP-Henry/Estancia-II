...
foreach ($actividades as $idActividad) {
    ...
    $sql_verificar = "SELECT * FROM avances WHERE IdPacienteAv = $idPaciente AND CedulaAv = '$cedulaPsicologo' AND (Actividad IS NULL OR Actividad = '') LIMIT 1";
    ...
    if (!$sql_actualizar or !$sql_insertar) {
        echo "<script>alert('Actividad asignada');window.location.href='../indexPsicologo.php';</script>";
    
    }else{
        echo "<script>alert('Actividad no aisgnada');window.location.href='../indexPsicologo.php';</script>";
    }
